...
namespace E_APP.SERVICES.SCIENCE_SERVICES.CHEMISTORY_SERVICES
{
    internal class Chemistory_Services01
    {
        ...
        private static Sqlite_Chemistry_Services01 Sqlite_Chemistry_Serv01 = new Sqlite_Chemistry_Services01();    

        public Chemistory_Services01()
        {
        
            load_find_Element_Textfile().Wait();
            category_elements_table().Wait();
        }
        ...
        private async Task load_find_Element_Textfile()
        {
            ...
            for (int i = 0; i < atomic_number.Count; i++)
            {
               ...
               atomic_number[i],
                ...
                var result01 = await Sqlite_Serv02.insert_element(
                        atomic_number[i],
                        element_name[i],
                        element_symboles[i],
                        atomic_mass[i],
        ...
        public async Task<string> find_element_using_sqlite(string input)
        {
            ...
            if (Security_Serv01.string_only_digit(input) == true)
            {
                ...
                {
                    data01[4] = atomic_number_result.output;
                }
            ...
            else
            {
                ...
                {
                    data01[4] = symbol_result.output;
                }
                else if (name_result.sucess == true)
                {
                    data01[4] = name_result.output;
                }
                else
                {
                    data01[4] = "failed";
        ...
        public async Task<string> find_category_elements_using_sqlite(int input)
        {
            ...
            {
                data01[7] = number01.output;

            }
            else if (number02.sucess == true)
            {
                data01[7] = number02.output;
            }
            else if (number03.sucess == true)
            {
                data01[7] = number03.output;
            }
            else if (number04.sucess == true)
            {
                data01[7] = number04.output;
            }
            else if (number05.sucess == true)
            {
                data01[7] = number05.output;
            }
            else if (number06.sucess == true)
            {
                data01[7] = number06.output;
            }
            else if (number07.sucess == true)
            {
                data01[7] = number07.output;
            }
            else
            {
                data01[7] = "IDK";
        ...
        private async Task category_elements_table()
        {
      ...
      { Alkali_Metals.Count,
            ...
            {
                var resault01 = await Sqlite_Chemistry_Serv01.insert_element_category(Alkaline_Earth_Metals[b], Alkaline_Earth_Metals[b],
                    Transition_Metals[b],
                    Lanthanides_Rare_Earth_Metals[b],
                    Actinides[b],
                    Noble_Gases[b],
                    Nonmetal_Gases_at_Room_Temperature[b]

);
                if (resault01.sucess == true)
                {
                    data01[22] += resault01.output;
