...
namespace E_APP.SERVICES.FILE_SERVICES.TEXT_FILES
{
    internal class Read_Textfiles
    {
        ...
        private static Stream[] stream01 = new Stream[file_helper.Length];
        private static StreamReader[] reader01 = new StreamReader[file_helper.Length];

        static Read_Textfiles()
        {
            for (int i = 0; i < file_helper.Length; i++)
            {
                stream01[i] = assembly.GetManifestResourceStream(file_helper[i])
                    ?? throw new InvalidOperationException($"Resource '{file_helper[i]}' not found.");
                reader01[i] = new StreamReader(stream01[i]);
            }
        }

        private static string[] data01 = {
            reader01[0].ReadToEnd(),
