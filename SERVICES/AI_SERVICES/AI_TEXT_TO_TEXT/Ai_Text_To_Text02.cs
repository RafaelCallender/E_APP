...
namespace E_APP.SERVICES.AI_SERVICES.AI_TEXT_TO_TEXT
{
    internal class Ai_Text_To_Text02
    {
        ...
        public async Task<string> text_to_text_content01(string input, Action? chunkLoader,int saved_text)
        {
            ...
            string prompt = $"""
You are a **King James Bible scholar and text interpreter**.
You will answer questions *only* using the Bible text.

### RULES
1. Do **NOT** use any outside knowledge other than the Bible Text and Saved Text.
...
4. When quoting scripture, include chapter and verse if present in the Bible text.
...
### SAVED TEXT
{File_Helper01.file_path_data01[(int)File_Helper01.file_path_data01_enum.bible].Trim()}
