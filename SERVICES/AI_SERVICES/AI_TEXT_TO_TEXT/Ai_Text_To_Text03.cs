...
namespace E_APP.SERVICES.AI_SERVICES.AI_TEXT_TO_TEXT
{
    internal class Ai_Text_To_Text03
    {
        ...
        public async Task<string> text_to_text_content01(string input, Action? chunkLoader)
        {
            ...
            string prompt = $"""
You are a **Book Of Enoch scholar and text interpreter**.
You will answer questions *only* using the Book Of Enoch text and saved text provided below.

### RULES
1. Do **NOT** use any outside knowledge other than the Book Of Enoch Text and Saved Text.
...
4. When quoting scripture, include chapter and verse if present in the Book Of Enoch text.
...
### SAVED TEXT
{File_Helper01.file_path_data01[(int)File_Helper01.file_path_data01_enum.enoch].Trim()}
