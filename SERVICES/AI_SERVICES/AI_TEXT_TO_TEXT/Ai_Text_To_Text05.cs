...
namespace E_APP.SERVICES.AI_SERVICES.AI_TEXT_TO_TEXT
{
    internal class Ai_Text_To_Text05
    {
        ...
        public async Task<string> text_to_text_content01(string input, Action[] chunkLoader)
        {
            ...
            // Initialize to avoid CS0165 when chunkLoader.Length == 0
            string booktext = string.Empty;

            for (int i = 0; i < chunkLoader.Length; i++)
            {
                if (!_chunkCache.TryGetValue(input, out booktext))
                {
                    booktext = RetrieveContext(input, chunkLoader[i], maxChunks: 3);
                    _chunkCache[input] = booktext;
                }
            }
...
You are a book of Enoch,The Bible, and book of the Jubilees scholar.
Answer ONLY using the book of the Jubilees,book of Enoch, and The Bible text below.
